...
(() => {
  ...
  const empty = () => ({
    schema: "1", read_only: true, source: "no-snapshot", evidence: {}, decision: {}, orchestrator: {}, session: {}, profiles: [], plugins: {}, plugin_health: {}, evidence_engine: {}, analysis: {}, policy: {}, action: {}, action_gate: {}, control_plane: {}, device_compatibility: {}, safety: {}, version: UNKNOWN
  });
  ...
  function safeSnapshot(raw) {
    ...
    const legacyAction = unified ? raw.action : gaming.action;
    const capabilityAction = unified ? raw.capability_action : gaming.capability_action;
    if (legacyAction !== undefined && (!legacyAction || typeof legacyAction !== "object" || Array.isArray(legacyAction))) throw new Error("Invalid action section.");
    if (capabilityAction !== undefined && (!capabilityAction || typeof capabilityAction !== "object" || Array.isArray(capabilityAction))) throw new Error("Invalid capability action section.");
    snapshot.action = capabilityAction || legacyAction || {};
    ...
    const deviceCompatibility = unified ? raw.device_compatibility : gaming.device_compatibility;
    if (deviceCompatibility !== undefined && (!deviceCompatibility || typeof deviceCompatibility !== "object" || Array.isArray(deviceCompatibility))) throw new Error("Invalid device compatibility section.");
    snapshot.device_compatibility = deviceCompatibility || {};
