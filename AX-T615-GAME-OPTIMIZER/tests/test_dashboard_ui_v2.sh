...
contains "$HTML" 'Action Safety Gate' "dashboard includes the simulation-only Action Safety Gate section"
for field in actionGateState actionGateReason actionGateMode actionGateSimulation actionGateRecommendation actionGateConfidence actionGateEvidenceQuality actionGatePolicyState actionGateRequestSource actionGateRealAction actionGateAuditCount actionGateAuditBound actionGateTimestamp; do contains "$HTML" "id=\"${field}\"" "action-gate section exposes ${field}"; done
