...
Orchestrator evidence → bounded analysis → fixed policy → recommendation → controlled action plan → validation → dry run → explicit apply → verification → rollback
...
| `sh bin/vegas action status` | Report the controlled-action architecture, dry-run default, emergency-lock state, fixed action registry, and managed-state-only scope. |
| `sh bin/vegas action snapshot` | Emit the fixed controlled-action envelope with plan, validation, lock, bounded audit, rollback, and no-hardware-control fields. |
| `sh bin/vegas action capabilities` | List the immutable action and blocked-action registries, explicit apply requirement, and hardware/process/network prohibitions. |
| `sh bin/vegas action plan` | Produce a deterministic dry-run plan for the policy-selected fixed action identifier. |
| `sh bin/vegas action validate` | Evaluate the fixed safety gates without applying a managed-state change. |
| `sh bin/vegas action dry-run` | Render the validated, non-applying action result. |
| `sh bin/vegas action apply` | Attempt the single fixed managed-state action only after an explicit healthy-evidence unlock; it remains blocked by default. |
| `sh bin/vegas action rollback` | Remove only the engine-owned managed marker and append a bounded audit record. |
| `sh bin/vegas action {history|lock|unlock}` | Inspect bounded local audit records, restore the emergency lock, or request an evidence-gated managed-action unlock. |
...
The AX-T615 plugin exposes only established observability operations: status, capabilities, games, game detection, profiles, FPS and performance analysis, memory/thermal/power telemetry, orchestrator status, dashboard access, fixed evidence output, fixed bottleneck analysis, fixed policy output, fixed controlled-action status, and dry-run reports. The independent System Observer plugin exposes only `status`, `capabilities`, `inspect`, and `snapshot`; its fixed adapter reports application version, OS/kernel/architecture, a sanitized hostname when safe, uptime, and available memory summary. Performance Observer exposes the same fixed operation set and normalizes only existing AX-T615 orchestrator evidence across CPU, GPU, memory, thermal, FPS, battery, and power categories. Its unavailable values remain `UNKNOWN`, its provenance is declared, and it reports no derived values. The Phase 4 unified layer composes the three fixed adapters into a bounded `vegas snapshot`; it neither executes plugin-provided paths nor re-implements AX-T615 policy logic. Phase 6 adds a fixed evidence engine that classifies each metric as `VALID`, `UNKNOWN`, `UNAVAILABLE`, `STALE`, or `INVALID`, declares freshness, provenance, confidence, quality, and only a fixed small non-sensitive history for deterministic trends. Phase 7 adds the fixed `bin/bottleneck-engine`, exposed through `vegas analysis` and the AX-T615 adapter’s allowlisted `analysis` operation. Phase 8 adds the fixed `bin/policy-engine`, exposed through `vegas policy` and the adapter’s allowlisted `policy` operation. Phase 9 adds the fixed `bin/action-engine`, exposed through `vegas action` and the adapter’s allowlisted `action` operation. Its immutable registry can affect only one VEGAS-inject runtime marker; all hardware, system, process, network, game, charging, display, thermal, memory, and arbitrary-command categories are blocked. Existing `AXGO_*` paths, variables, configuration, runtime, and module identifiers are retained.

## Evidence, analysis, policy, recommendation, and controlled action

The product maintains a deliberately constrained safety boundary. **Evidence** is normalized, provenance-labeled observation. **Analysis** is a deterministic explanation of which supported bottleneck signal may be present, plus bounded trend context and confidence. **Policy** selects a recommendation-only state using the documented safety order: safety evidence, thermal, memory, battery/power, performance, then profile preference. **Recommendation** is a read-only instruction to inspect, monitor, collect more evidence, remain conservative, or consider a named logical profile. **Controlled action** is a fixed engine-owned managed-state transition, never an arbitrary command or device control: the default is dry run with the emergency lock enabled, and explicit apply is allowed only after validated fresh safety evidence permits an unlock.

> `Evidence → Analysis → Policy → Recommendation → Action Plan → Validation → Dry Run → Explicit Apply → Verification → Rollback` never crosses into hardware, process, game, network, charging, display, thermal, memory, or system control. The `analysis`, `policy`, and `action` envelopes cannot authorize an arbitrary executable path. The only mutable target is a repository-local VEGAS-inject managed marker, which is recoverable through a fixed rollback.

## Safety guarantees

VEGAS-inject evidence, analysis, policy, and all bundled plugins are **read-only and recommendation-only**. The Phase 9 controlled-action engine can write only its own repository-local runtime marker, lock, and bounded audit records; it cannot write `/proc` or `/sys`, alter CPU/GPU governors or frequencies, change Android properties, Power HAL behavior, display mode, charging, battery controls, thermal policy, ZRAM, swap, LMKD/OOM settings, or processes. It does not kill or force-stop applications, execute profile/telemetry/metadata content, inject into games, or expose raw hardware-control commands. **Hardware control capabilities are `BLOCKED`.** System Observer does not collect personal files, credentials, account data, messages, persistent identifiers, or arbitrary environment variables. Performance Observer does not access the network, run arbitrary commands, or infer missing performance values.
