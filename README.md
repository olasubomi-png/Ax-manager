...
Orchestrator evidence → bounded analysis → fixed policy → recommendation → action safety gate → simulated result
        ↓ separate fixed capability-gated action planner
        ↓ dry-run validation and bounded local audit only
        ↓ fixed read-only composition
VEGAS Control Plane → validated JSON snapshot → [real device apply: NOT AVAILABLE]
...
| `sh bin/vegas action status` | Report the fixed capability-gated action planner, its dry-run-only mode, evidence/policy state, bounded audit availability, and no-real-device-apply contract. |
| `sh bin/vegas action snapshot` | Emit the fixed capability-gated action snapshot; it is distinct from the legacy Action Safety Gate simulation record. |
| `sh bin/vegas action plan <fixed-action>` | Describe a compiled-in action schema and preconditions without applying an action. |
| `sh bin/vegas action validate <fixed-action>` | Validate only the fixed ID, capability, evidence, policy, confidence, and safety-classification gates. |
| `sh bin/vegas action dry-run <fixed-action>` | Create at most a bounded repository-local planning audit record; no device state changes. |
| `sh bin/vegas action apply <fixed-action> --dry-run` | Alias only to the dry-run route; every real apply spelling is denied. |
| `sh bin/vegas action verify <fixed-action>` | Report `NOT_APPLIED` because this release has no real device action to verify. |
| `sh bin/vegas action rollback <fixed-action>` | Report `NO_APPLIED_ACTION` because this release has no real device action to reverse. |
